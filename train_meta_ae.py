...
def metatrain_fn(encoder,
    ...
    for idx, x_list in enumerate(zip(*loaders)):
        # Phase 1: Fix enc, train different dec with a fixed batch
        if idx == 0:
            x_list_fixed = x_list
        ...
        # Phase 2: Fix dec, train meta enc
        for i, (x, k) in enumerate(x_list):
            _, loss = forward_fn(encoder, decoders[i], x, k)
            encoder_optimizer.zero_grad()
            loss.backward()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--metatrain_lr', type=float, default=1e-3)
    # ----- Phase3: Finetune -----
    # Dataset parameters
    ...
    args.logdir = os.path.join(
        args.logdir, 'MetaAE_' + datetime.now().strftime('%Y%m%d%H%M%S'))
    set_seed(args.seed)
